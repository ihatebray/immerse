 ...
 *
 *  Lifecycle:
 *    1. App boots → initAutoUpdater() fires a check against the GitHub
 *       Release feed configured in package.json's `build.publish` block.
 *    2. If a newer version exists, the installer downloads silently in
 *       the background. We forward progress events to the renderer so
 *       it can show a small toast/badge if it wants to.
 *    3. When the download finishes, we send an 'update:downloaded'
 *       event. The renderer shows a "Restart to install" toast. Clicking
 *       it invokes 'update:install', which calls quitAndInstall().
 *    4. Hourly re-checks while the app is open (in case the user leaves
 *       it running for days).
 *
 *  IPC channels:
...
// Lazy require — keeps the module out of the cold-start path if anything
// goes wrong with the import. The package isn't an Electron API, it's
// pure Node, so it works fine inside main.js.
...
async function initAutoUpdater() {
  if (autoUpdater) return; // idempotent
  try {
    // eslint-disable-next-line global-require
    const { autoUpdater: au } = require('electron-updater');
    autoUpdater = au;
  } catch (e) {
    console.warn('[updater] electron-updater not installed:', String(e?.message || e));
    return;
  }

  // Tell electron-updater where to fetch releases from. Without an
  // explicit feed URL, the library tries to read app-update.yml from
  // the installation directory's resources/ folder — that file is
  // generated by electron-builder's installers but NOT by Squirrel
  // (which is what Forge uses). Without it, the updater throws ENOENT
  // on every check.
  //
  // ⚠️ Replace YOUR_GITHUB_USERNAME below with your actual username.
  // The 'immerse-releases' repo name should match whatever repository
  // your publisher uploads to (single-repo setup uses 'immerse', the
  // two-repo private-code setup uses 'immerse-releases').
  try {
    ...
    console.warn('[updater] setFeedURL failed:', String(e?.message || e));
  }

  // Don't download silently until we say so. We DO want auto-download
  // for the "just works" UX, but we want explicit control over the
  // install moment (user-initiated via the toast button).
  autoUpdater.autoDownload = true;
  autoUpdater.autoInstallOnAppQuit = false;
  // Log to electron-updater's default logger location for debugging.
  // It writes to ~/.config/immerse/logs on Linux, %APPDATA%\immerse\logs
  // on Windows, ~/Library/Logs/immerse on macOS.
  ...
  autoUpdater.on('download-progress', (p) => {
    emitUpdaterStatus({
      state: 'downloading',
      progressPct: Math.round(p?.percent ?? 0),
    });
  });
  autoUpdater.on('update-downloaded', (info) => {
    emitUpdaterStatus({
      state: 'downloaded',
      version: info?.version || updaterStatus.version,
      progressPct: 100,
      error: '',
    });
  });
  autoUpdater.on('error', (err) => {
    // Network errors are very common (offline, GitHub flake) and not
    // worth surfacing as scary banners. We track them in status but
    // the renderer treats 'error' gently — a small "couldn't check"
    // line rather than a red alert.
    emitUpdaterStatus({
      state: 'error',
  ...
  // Initial check on boot — delayed slightly so the renderer is ready
  // to receive the status event when it fires.
  setTimeout(() => {
    autoUpdater.checkForUpdates().catch((e) => {
      console.warn('[updater] initial check failed:', String(e?.message || e));
    });
  }, 3000);

  // Hourly recheck for long-running sessions. checkForUpdates is cheap
  // (one HTTP GET to latest.yml) so this is fine.
  setInterval(() => {
    if (updaterStatus.state === 'downloading' || updaterStatus.state === 'downloaded') return;
...
ipcMain.handle('update:install', () => {
  ...
  // quitAndInstall(isSilent, isForceRunAfter). Defaults are (true, true)
  // — silent install on Windows, app relaunches on completion. Same
  // behavior we want.
...
async function ingestSoulseekFile({ downloadedPath, originalFilename }) {
  ...
  const track = {
    ...
    year,
    genre: parsed.genre || '',
    explicit: typeof parsed.explicit === 'boolean' ? parsed.explicit : null,
  };
